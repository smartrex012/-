...
    2.  **종합 브리핑 (필수):** 인사말 후에, ${data.locationName}의 ${data.forecastHour} 날씨를 바탕으로 **행동 지침**과 **옷차림 추천**을 **하나의 자연스러운 문단**으로 이어서 설명해주세요. 
    3.  **데이터 반영 (필수):** 옷차림 추천 시, [일교차 정보]와 [체감온도 정보]를 반드시 말로 풀어서 반영하세요. (예: "일교차가 크니 겉옷을 챙기세요", "바람이 약해 실제 기온과 비슷할 거예요.")
    4.  **날씨 요약 (필수):** 종합 브리핑이 끝난 후, 한 줄을 띄우고 '[${data.locationName} (${data.forecastHour}) 날씨 요약]'이라는 제목을 붙인 뒤, 아래 항목들을 **간단한 목록 형식** (예: '* 기온: 7℃')으로 요약하세요. 쉼표나 표 형식을 절대 사용하지 마세요.
        * 기온: ${data.temp}℃
        * 하늘 상태: ${skyText}
        * 강수 확률: ${data.precipProb}%
        * 강수 형태: ${precipText}
        * 체감 온도: ${windChillText}
        * 일교차: ${tempRangeText}
    5.  **마무리 이모지:** 요약 목록 아래에 날씨에 어울리는 ☀️, ☁️, 🌧️ 같은 이모지 1개를 붙이며 마무리하세요.
  `;
  
