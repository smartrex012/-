...
    4.  **데이터 반영 (필수):** 옷차림 추천 시, [일교차 정보]와 [체감온도 정보]를 반드시 말로 풀어서 반영하세요. (예: "일교차가 크니 얇은 겉옷을 챙기세요", "바람이 불어 체감온도가 낮으니 목도리가 좋겠어요"). 날씨에 어울리는 ☀️, ☁️, 🌧️ 같은 이모지 1개를 붙이며 마무리하세요.
    5.  **날씨 요약 (필수):** 종합 브리핑이 끝난 후, 한 줄을 띄우고 '[${data.locationName} (${data.forecastHour}) 날씨 요약]'이라는 제목을 붙인 뒤, 아래 항목들을 **간단한 목록 형식** (예: '* 기온: 7℃')으로 요약하세요. 쉼표나 표 형식을 절대 사용하지 마세요.
        * 기온: ${data.temp}℃
        * 하늘 상태: ${skyText}
        * 강수 확률: ${data.precipProb}%
        * 강수 형태: ${precipText}
