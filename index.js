...
**[ 1단계: 본인의 Discord 고유 ID 복사하기 ]**
\`${member.id}\`
(방금 입장하신 ${member.user.username}님의 Discord 고유 ID입니다. 위 ID를 복사(Ctrl+C)해주세요.)

**[ 2단계: 위치 등록하기 (필수) ]**
아래 Google Form 링크를 열고, 방금 복사한 **Discord 고유 ID**와 **'날씨가 궁금하신 동네 이름'**을 입력해 주세요.
(정확한 '행정동' 이름 (예: 잠실1동, 목3동)을 입력하시면 가장 정확한 예보를 받으실 수 있습니다.)
> ${GOOGLE_FORM_URL}

등록이 완료되면, #일반 채널에서 \`/weather\` 명령어를 사용하실 수 있습니다. 이 명령어는 요청하신 동네의 날씨 보고서를 받아볼 수 있게 합니다!
`;
...
이제 날씨 알리미 서버의 #일반 채널에서 '/weather' 명령어를 입력하시면,
등록하신 위치의 최신 날씨 보고서를 **DM(개인 메시지)**으로 보내드립니다.
`;
